import numpy as np
import cv2
from .model import Models

class EyeProcessor:
    """
    Process eye and send to model
    """
    def __init__(self, model = Models()):
        self.model = model.load_eye_model()

        self.LEFT_EYE_INDICES = [362, 382, 381, 380, 374, 373, 390, 249, 263, 466, 388, 387, 386, 385, 384, 398] # Landmarks for eye left
        self.RIGHT_EYE_INDICES = [33, 7, 163, 144, 145, 153, 154, 155, 133, 173, 157, 158, 159, 160, 161, 246] # Landmarks for eye right
        self.PREDICTION_THRESHOLD = 0.3

    def __extract_eye_landmarks(self,frame_shape, face_landmarks, eye_type='left'):
        """
        Extract landmarks of eye
        Args: face_landmarks, eye_type - eye 'left' or 'right'
        Returns: list of (x, y) coordinates
        """
        height, width = frame_shape[:2]
        indices = self.LEFT_EYE_INDICES if eye_type == 'left' else self.RIGHT_EYE_INDICES
        eye_landmarks = []
        for i in indices:
            if i < len(face_landmarks.landmark):
                x = int(face_landmarks.landmark[i].x * width)
                y = int(face_landmarks.landmark[i].y * height)
                eye_landmarks.append((x, y))
        return eye_landmarks
    
    def __crop_eye_region(self, frame, eye_landmarks):
        """
        Crop eye region to frame
        Args: frame, eye_landmark
        Returns: cropped eye image
        """

        xs, ys = zip(*eye_landmarks) # Zip landmarks
        x_min, x_max = int(min(xs)), int(max(xs)) # Choose min and max of X landmarks
        y_min, y_max = int(min(ys)), int(max(ys)) # Choose min and max of Y landmarks

        crop = frame[y_min:y_max, x_min:x_max]
        if crop.size == 0:
            return None
        return crop
    
    def __process_eye(self, eye_image):
        """
        Process image for model
        Args: eye_image
        Returns: image process
        """
        if eye_image is None:
            return None
        if len(eye_image.shape) == 2:
            eye_image = cv2.cvtColor(eye_image, cv2.COLOR_GRAY2BGR)
        eye_resized = cv2.resize(eye_image, (80,80)) # Resize image to 80x80
        eye_array = eye_resized.astype(np.float32) / 255.0 # Normalize 0-1
        eye_array = np.expand_dims(eye_array, 0) # Add batch dimension
        return eye_array
    
    def __predict_single_eye_prob(self, processed_eye) -> float:
        """
        Dự đoán và trả về xác suất MỞ của một mắt từ model
        """
        if processed_eye is None:
            return 1.0
        prediction = self.model.predict(processed_eye, verbose=0)
        return prediction[0][0]
    
    def detect_eyes(self, frame, face_landmarks):
        """
        Phát hiện xác suất mắt mở cho từng mắt, sau đó tính trung bình.
        Trả về: avg_prob_open (float)
        """
        rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

        prob_open_left = 1.0 # Mặc định là mở nếu không phát hiện được mắt
        left_eye_landmarks = self.__extract_eye_landmarks(rgb_frame.shape, face_landmarks, eye_type='left')
        if left_eye_landmarks:
            left_img_cropped = self.__crop_eye_region(rgb_frame, left_eye_landmarks)
            left_img_processed = self.__process_eye(left_img_cropped)
            if left_img_processed is not None:
                prob_open_left = self.__predict_single_eye_prob(left_img_processed)

        prob_open_right = 1.0 # Mặc định là mở nếu không phát hiện được mắt
        right_eye_landmarks = self.__extract_eye_landmarks(rgb_frame.shape, face_landmarks, eye_type='right')
        if right_eye_landmarks:
            right_img_cropped = self.__crop_eye_region(rgb_frame, right_eye_landmarks)
            right_img_processed = self.__process_eye(right_img_cropped)
            if right_img_processed is not None:
                prob_open_right = self.__predict_single_eye_prob(right_img_processed)

        avg_prob_open = (prob_open_left + prob_open_right) / 2.0
        return avg_prob_open